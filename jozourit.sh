#!/bin/bash
#/ Usage: jozourit.sh [-vh]
#/
#/ Install Jozour-IT asset management.
#/
#/ OPTIONS:
...
readonly APP_USER="jozouritapp"
readonly APP_NAME="jozourit"
...
log () {
  if [ -n "$verbose" ]; then
    eval "$@" |& tee -a /var/log/jozourit-install.log
  else
    eval "$@" |& tee -a /var/log/jozourit-install.log >/dev/null 2>&1
  fi
}
...
create_user () {
  echo "* Creating Jozour-IT user."

  if [[ "$distro" == "ubuntu" ]] || [[ "$distro" == "debian" ]] || [[ "$distro" == "raspbian" ]] ; then
    adduser --quiet --disabled-password --gecos 'Jozour-IT User' "$APP_USER"
  else
    adduser "$APP_USER"
  fi
...
install_jozourit () {
  ...
  mysql -u root -p --execute="CREATE DATABASE jozourit;GRANT ALL PRIVILEGES ON jozourit.* TO jozourit@localhost IDENTIFIED BY '$mysqluserpw';"

  echo "* Cloning Jozour-IT from github to the web directory."
  log "git clone https://github.com/jozour/jozour-it $APP_PATH"

  echo "* Configuring .env file."
  cp "$APP_PATH/.env.example" "$APP_PATH/.env"

  #TODO escape SED delimiter in variables
  sed -i '1 i\#Created By Jozour-it Installer' "$APP_PATH/.env"
  sed -i "s|^\\(APP_TIMEZONE=\\).*|\\1$tzone|" "$APP_PATH/.env"
  sed -i "s|^\\(DB_HOST=\\).*|\\1localhost|" "$APP_PATH/.env"
  sed -i "s|^\\(DB_DATABASE=\\).*|\\1jozourit|" "$APP_PATH/.env"
  sed -i "s|^\\(DB_USERNAME=\\).*|\\1jozourit|" "$APP_PATH/.env"
...
rename_default_vhost () {
    ...
    log "mv /etc/apache2/sites-enabled/jozourit.conf /etc/apache2/sites-enabled/000-jozourit.conf"
}
...
echo ""
echo "  Welcome to Jozour-IT Inventory Installer for CentOS, Rocky, Fedora, Debian and Ubuntu!"
echo ""
shopt -s nocasematch
case $distro in
  *ubuntu*)
...
case $setpw in
  [yY] | [yY][Ee][Ss] )
    ...
    ;;
  [nN] | [n|N][O|o] )
    echo -n  "  Q. What do you want your jozourit user password to be?"
    read -rs mysqluserpw
    echo ""
    ans="no"
    ;;
...
echo ""
echo "  ***Open http://$fqdn to login to Jozour-IT.***"
echo ""
echo ""
echo "* Cleaning up..."
rm -f jozourit.sh
