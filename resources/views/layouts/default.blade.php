...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
  <head>
    ...
    <meta content="width=device-width, initial-scale=1" name="viewport">
      ...
      <link rel="apple-touch-icon" href="{{ ($jozourSettings) && ($jozourSettings->favicon!='') ?  Storage::disk('public')->url(e($jozourSettings->logo)) :  config('app.url').'/img/logo.png' }}">
      <link rel="apple-touch-startup-image" href="{{ ($jozourSettings) && ($jozourSettings->favicon!='') ?  Storage::disk('public')->url(e($jozourSettings->logo)) :  config('app.url').'/img/logo.png' }}">
      <link rel="shortcut icon" type="image/ico" href="{{ ($jozourSettings) && ($jozourSettings->favicon!='') ?  Storage::disk('public')->url(e($jozourSettings->favicon)) : config('app.url').'/favicon.ico' }} ">
    ...
    @if (($jozourSettings) && ($jozourSettings->allow_user_skin==1) && Auth::check() && Auth::user()->present()->skin != '')
        <link rel="stylesheet" href="{{ url(mix('css/dist/skins/skin-'.Auth::user()->present()->skin.'.min.css')) }}">
    @else
    <link rel="stylesheet" href="{{ url(mix('css/dist/skins/skin-'.($jozourSettings->skin!='' ? $jozourSettings->skin : 'blue').'.css')) }}">
    @endif
    {{-- page level css --}}
    @stack('css')
    ...
    <style nonce="{{ csrf_token() }}">
        ...
        .skin-{{ $jozourSettings->skin!='' ? $jozourSettings->skin : 'blue' }} .sidebar-menu > li:hover > a, .skin-{{ $jozourSettings->skin!='' ? $jozourSettings->skin : 'blue' }} .sidebar-menu > li.active > a {
    ...
    <script nonce="{{ csrf_token() }}">
          window.jozourit = {
              settings: {
                  "per_page": {{ $jozourSettings->per_page }}
              }
          };
    </script>
  ...
  @if (($jozourSettings) && ($jozourSettings->allow_user_skin==1) && Auth::check() && Auth::user()->present()->skin != '')
      <body class="sidebar-mini skin-{{ $jozourSettings->skin!='' ? Auth::user()->present()->skin : 'blue' }} {{ (session('menu_state')!='open') ? 'sidebar-mini sidebar-collapse' : ''  }}">
  @else
      <body class="sidebar-mini skin-{{ $jozourSettings->skin!='' ? $jozourSettings->skin : 'blue' }} {{ (session('menu_state')!='open') ? 'sidebar-mini sidebar-collapse' : ''  }}">
  @endif
    ...
    <div class="wrapper">
              ...
              <div class="left-navblock">
                 @if ($jozourSettings->brand == '3')
                      <a class="logo navbar-brand no-hover" href="{{ url('/') }}">
                          ...
                          <img class="navbar-brand-img" src="{{ Storage::disk('public')->url($jozourSettings->logo) }}" alt="{{ $jozourSettings->site_name }} logo">
                          @endif
                          {{ $jozourSettings->site_name }}
                      </a>
                  @elseif ($jozourSettings->brand == '2')
                      <a class="logo navbar-brand no-hover" href="{{ url('/') }}">
                          @if ($jozourSettings->logo!='')
                            <img class="navbar-brand-img" src="{{ Storage::disk('public')->url($jozourSettings->logo) }}" alt="{{ $jozourSettings->site_name }} logo">
                          @endif
                          <span class="sr-only">{{ $jozourSettings->site_name }}</span>
                      </a>
                  @else
                      <a class="logo navbar-brand no-hover" href="{{ url('/') }}">
                          {{ $jozourSettings->site_name }}
          ...
          <!-- Navbar Right Menu -->
            <div class="navbar-custom-menu">
              <ul class="nav navbar-nav">
               ...
               @can('admin')
               @if ($jozourSettings->show_alerts_in_menu=='1')
      ...
      <footer class="main-footer hidden-print">
        ...
        <div class="pull-right hidden-xs">
          @if ($jozourSettings->version_footer!='off')
              @if (($jozourSettings->version_footer=='on') || (($jozourSettings->version_footer=='admin') && (Auth::user()->isSuperUser()=='1')))
                &nbsp; <strong>Version</strong> {{ config('version.app_version') }} - build {{ config('version.build_version') }} ({{ config('version.branch') }})
              @endif
          @endif

          @if ($jozourSettings->support_footer!='off')
              @if (($jozourSettings->support_footer=='on') || (($jozourSettings->support_footer=='admin') && (Auth::user()->isSuperUser()=='1')))
                <a target="_blank" class="btn btn-default btn-xs" href="https://jozour-it.readme.io/docs/overview" rel="noopener">{{ trans('general.user_manual') }}</a>
                <a target="_blank" class="btn btn-default btn-xs" href="https://jozouritapp.com/support/" rel="noopener">{{ trans('general.bug_report') }}</a>
                 @endif
          @endif

        @if ($jozourSettings->privacy_policy_link!='')
            <a target="_blank" class="btn btn-default btn-xs" rel="noopener" href="{{  $jozourSettings->privacy_policy_link }}" target="_new">{{ trans('admin/settings/general.privacy_policy') }}</a>
        @endif


        </div>
          @if ($jozourSettings->footer_text!='')
              <div class="pull-right">
                  {!!  Parsedown::instance()->text(e($jozourSettings->footer_text))  !!}
              </div>
          @endif
          

          <a target="_blank" href="https://jozouritapp.com" rel="noopener">Jozour-IT</a>.
      </footer>
