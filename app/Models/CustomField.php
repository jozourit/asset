...
class CustomField extends Model
{
     ...
     * do with previously existing values. - @jozour
     *
     * @author [A. Gianotto] [<jozour@jozour.net>]
     * @since [v3.4]
     * @return string
     */
    public static function name_to_db_name($name)
    {
        return '_jozourit_'.preg_replace('/[^a-zA-Z0-9]/', '_', strtolower($name));
    }
    ...
    public function convertUnicodeDbSlug($original = null)
    {
        ...
        if (! function_exists('transliterator_transliterate')) {
            $long_slug = '_jozourit_'.str_slug(\Patchwork\Utf8::utf8_encode(trim($name)), '_');
        } else {
            $long_slug = '_jozourit_'.Utf8Slugger::slugify($name, '_');
        }
     ...
     * @see https://github.com/jozour/jozour-it/issues/5896
     *
